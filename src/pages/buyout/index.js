...
class Orderdetail extends Component {
  ...
  render() {
    ...
    const { cashes, product, userAddress, userOrders, totalBuyPrice,totalBuyPriceFinal,payRent,loading } = this.props;
    const createTiemStr = userOrders.createTime && formatDate(new Date(userOrders.createTimeStr), 'yyyy年MM月dd日 hh:mm');
    const paymentTimeStr = userOrders.paymentTime && formatDate(new Date(userOrders.paymentTimeStr), 'yyyy年MM月dd日 hh:mm');
    ...
    return (
      <View className='orderDetail-page'>
          ...
          <View className='renewal-days'>
            <View className='border'></View>
                ...
                <View className='ren-day'>
                  ...
                  <View className='day black'><Text className='bol'>¥</Text>{totalBuyPrice  }</View>
                </View>
                <View className='ren-day'>
                  <View className='day'>已付租金</View>
                  <View className='day'><Text className='bol'>¥</Text>{payRent}</View>
                </View>
        ...
        <View className='order-info'>
          ...
          {userOrders.paymentTime && (
            <View className='gray-info'>
              <View className='left-text'>支付时间</View><View className='right-text'>{paymentTimeStr}</View>
            </View>
          )}
        </View>
        ...
        {userOrders.status === 'USER_CANCELED_CLOSED' && userOrders.type ===3 && (
          <View className='end-banner'>
            <View className='button-bar' onClick={this.handleClickRenewalBefore}>查看原订单</View>
            <View className='button-bar-active' onClick={this.connectService}>联系客服</View>
          </View>
        )}
        {userOrders.status === 'ORDER_FINISH' && userOrders.type ===3 && (
          <View className='end-banner'>
            <View className='button-bar' onClick={this.handleClickRenewalBefore}>查看原订单</View>
            <View className='button-bar-active' onClick={this.connectService}>联系客服</View>
          </View>
